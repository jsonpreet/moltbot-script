#!/usr/bin/env bash
# install-clawdbot-vps.sh — Single-script VPS install for Moltbot (clawdbot)
# Runs full moltbot onboard wizard; then locks down gateway to loopback + optional firewall/fail2ban.
# Usage: curl -fsSL https://raw.githubusercontent.com/.../install-clawdbot-vps.sh | bash
#        bash install-clawdbot-vps.sh [--no-fail2ban] [--no-firewall]
...
# --- Helpers ---
log() { echo "[clawdbot-install] $*"; }
err() { echo "[clawdbot-install] ERROR: $*" >&2; }
...
ensure_run_user() {
  if [[ "$(id -u)" -eq 0 ]]; then
    if ! id -u "$CLAWDBOT_USER" &>/dev/null; then
      log "Creating dedicated user: $CLAWDBOT_USER"
      useradd --system --create-home --shell /bin/bash "$CLAWDBOT_USER" 2>/dev/null || true
      if ! id -u "$CLAWDBOT_USER" &>/dev/null; then
        err "Could not create user $CLAWDBOT_USER; continuing as root (not recommended)."
        RUN_USER=""
        return
      fi
    fi
    ensure_clawdbot_sudoers
    RUN_USER="$CLAWDBOT_USER"
...
# Allow clawdbot user to run apt-get, ufw, systemctl (for install script when re-exec'd as clawdbot)
ensure_clawdbot_sudoers() {
  local sudoers_file="/etc/sudoers.d/99-clawdbot-install"
  [[ -f "$sudoers_file" ]] && return 0
  log "Allowing $CLAWDBOT_USER to run apt-get, ufw, systemctl (for install)"
  # NOPASSWD only for the commands the install script needs
  printf '%s ALL=(ALL) NOPASSWD: /usr/bin/apt-get, /usr/sbin/apt-get, /usr/sbin/ufw, /usr/bin/systemctl\n' "$CLAWDBOT_USER" > "$sudoers_file"
...
# --- Main (when not re-execing as clawdbot) ---
do_install() {
  local os id_node
  os=$(detect_os)
  ...
  # When running as root, create user and re-exec script as that user for the actual install
  ...
  CLAWDBOT_HOME="$(get_clawdbot_home)"
  CLAWDBOT_DIR="${CLAWDBOT_HOME}/.clawdbot"
  CLAWD_DIR="${CLAWDBOT_HOME}/clawd"

  # --- Required packages (curl, ca-certificates, jq, gnupg) ---
  ensure_required_packages "$os"

  # --- Node 22+ ---
  if ! command -v node &>/dev/null; then
    log "Node.js not found; installing Node 22 via NodeSource..."
...
[Unit]
Description=Moltbot Gateway (clawdbot)
...
EOF
  ...
  echo
  echo "=============================================="
  echo "  Clawdbot VPS install complete"
  echo "=============================================="
  echo
